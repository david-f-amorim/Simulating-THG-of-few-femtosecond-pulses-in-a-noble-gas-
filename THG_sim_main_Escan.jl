...
# ----------------- QUICK SETTINGS -------------------------------
e_scan = true               # if true, pressure scan is executed, with pressure range set by the variable "en_arr" below; if false, a single run is simulated
...
en_arr = range(start= 50e-6, stop= 50e-6, step= 500e-6)  # pressure range (only relevant for pressure scans)  [bar]
...
function THG_main(IRenergy=IRenergy) 
    ...
    if (save & !e_scan) == true              # create specific directory to store run output 
    ...
    if save == true 
        ...
        if e_scan == true 
            open(joinpath(out_path,string(IRenergy)*" uJ.dat"), "w") do file
...
if e_scan == true 
    ...
    printstyled("Starting energy scan on "*Dates.format(Dates.now(), "dd/mm/yyyy")*" at "*Dates.format(Dates.now(), "HH:MM:SS")*"\n", bold=true, color=:red, underline=true)
    ...
    for i in range(1,length(en_arr)) 
        printstyled("Starting run "*string(i)*"/"*string(length(en_arr))*" at "*Dates.format(Dates.now(), "HH:MM:SS")*"\n", bold=true, color=:green)

        E_UV, η, τ_UV, z_peak = THG_main(en_arr[i])

        open(joinpath(out_path,"energy_efficiency_time_zpeak.txt"), "a") do file
            writedlm(file, zip(en_arr[i], E_UV, η, τ_UV, z_peak))
        end
    end 
    
    printstyled("Finished energy scan on "*Dates.format(Dates.now(), "dd/mm/yyyy")*" at "*Dates.format(Dates.now(), "HH:MM:SS")*"\n", bold=true, color=:red, underline=true)
