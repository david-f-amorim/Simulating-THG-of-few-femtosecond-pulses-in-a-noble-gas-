...
% w_min_i = w_min;
% w_max_i = w_max;
[~,w_max_i] = min(abs(wavelength-w_min));
[~,w_min_i] = min(abs(wavelength-w_max));
...
subplot(122);
plot(wavelength,intensity_w./max(intensity_w));
ylim([0 1.15]);
xlim([min(wavelength) max(wavelength)]);
xlabel('Wavelength (nm)');
ylabel('Intensity (norm.)');
...
carrier_wavelength = sum(wavelength.*intensity_w)/sum(intensity_w)*1e9 %% in [nm]
...
figure('name','Transform limited calculator');
%set(gcf, 'Position', [100, 100, 600, 1000])
% plot(wavelength,intensity_w./max(intensity_w), linewidth=1.5, color='r');
area(wavelength,intensity_w./max(intensity_w), linewidth=1.5, EdgeColor='r', FaceColor=[0.996, 0.414,0.414]);
ylim([0 1.15]);
xlim([min(wavelength) max(wavelength)]);
xlabel('Wavelength (nm)');
ylabel('Intensity (norm.)');
